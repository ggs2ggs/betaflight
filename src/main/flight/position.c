...
void positionUpdate(void)
{
    ...
    // *** set debugs
    DEBUG_SET(DEBUG_ALTITUDE, 0, lrintf(altitudeCm));
    DEBUG_SET(DEBUG_ALTITUDE, 1, lrintf(baroAltCm));
    DEBUG_SET(DEBUG_ALTITUDE, 2, lrintf(gpsAltCm));
#ifdef USE_VARIO
    DEBUG_SET(DEBUG_ALTITUDE, 3, estimatedVario);
#endif
    DEBUG_SET(DEBUG_RTH, 1, lrintf(displayAltitudeCm));
    DEBUG_SET(DEBUG_BARO, 3, lrintf(baroAltCm));
}
