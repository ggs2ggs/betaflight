...
FAST_CODE_NOINLINE void calculateFeedforward(const pidRuntime_t *pid, int axis)
{
    ...
    if (axis == FD_ROLL) {
        DEBUG_SET(DEBUG_FEEDFORWARD, 1, lrintf(setpointSpeed * 0.1f));
        // crude setpoint delta
    }
    ...
    // smooth the acceleration element (effectively a second order filter)
    ...
    if (axis < FD_YAW) {
        ...
        // for yaw, add high-pass filtered setpoint to derivative based feedforward and acceleration
        ...
        // this provides a sustained, exponentially decaying FF on yaw that mimics natural motor activity
